...
const long gearRatio = 1;                  // Передаточное соотношение «Шаговый двигатель» : «Делительная голова»
...
void setGearTooth(int concat) {
  if (!runGear) {
    gearTooth = constrain(gearTooth + concat, 6, 360);
  }
}

void setDividerTotal(int concat) {
  if (!runDivider) {
    dividerTotal = constrain(dividerTotal + concat, 2, 360);
  }
}
